...
def main():
    ...
    print(f"    GPU: {gpu}")
    print()
    print(f"{model_load:>{7}}s model load speed")
    print()
    print("     Iterations per second @ 512x512:")
    print(f"{its:>{7}} basic inference (empirical)")
    print(f"{its_raw:>{7}} basic inference (theoretical)")
    print(f"{cnet_its:>{7}} controlnet inference (empirical)")
    print(f"{cnet_raw_its:>{7}} controlnet inference (theoretical)")
    print()
    print("     Details:")
    for name, data in timings.items():
        secs = round(data["end"] - data["start"], 2)
        print(f"{secs:>{7}}s {name}")
    print()
