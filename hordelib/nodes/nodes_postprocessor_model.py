...
class HordePPModelLoader:
    ...
    def load_model(self, model_name):
        ...
        if (
            SharedModelManager.manager.esrgan is not None
            and model_name in SharedModelManager.manager.esrgan.models
        ):
            return SharedModelManager.manager.esrgan.loaded_models[model_name]
        elif (
            SharedModelManager.manager.gfpgan is not None
            and model_name in SharedModelManager.manager.gfpgan.models
        ):
            return SharedModelManager.manager.gfpgan.loaded_models[model_name]
        else:
            logger.error(
                f"{model_name} not found in any of the PostProcessor Model Managers"
            )
            raise RuntimeError()  # XXX
